...
class LoginActivity : BaseActivity() {
    ...
    override fun initView() {
        btnLogin.setOnClickListener {
            ...
            if (verificationLoginParam(userAccount, userPwd)) {
                val loginParam = mapOf("userAccount" to userAccount, "userPassword" to userPwd)
                viewModel.getDataList(loginParam)
            }
        }
    }
    ...
    override fun initData() {
        viewModel.dataLiveData.observe(this) {
            if (it.isSuccess) {
                it.getOrNull()?.let { token -> viewModel.saveUserToken(token) }
            } else {
                getString(R.string.login_error_text).showToast()
            }
        }
    }
